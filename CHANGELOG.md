...
> 「工具的洞被当成公司的事实」。测试 491 → 510。

**Fixed**

- **PDF 段落抽取:引用被当成正文**。`mda` 命中的是 P.2「重要提示」里那句
  「详见第三节"管理层讨论与分析"的相关内容」(金山 353 字 / 华特 595 字),不是正文。
  旧的 500 字门槛只是**碰巧**挡住了它,而同一个门槛又把真正短的正文判成没抓到
  (金山「主要控股参股公司分析」正文 287 字,两家子公司的表完整在内)。
  换成字符级三分类,不再按长度猜:
  **目录**=标题后同一行跟点号引导+页码 / **引用**=紧挨着引号或前面压着一整句话 /
  **正文**=独占一行、前缀只有章节序号。短正文照样 `found=true`,只在 desc 里提示回原文核;
  `found=false` 留给「全文只在目录或引用里出现过」。
  实测:金山 mda P.2/353 字 → P.10/41691 字,华特 P.2/595 字 → P.13/10264 字。
- **半年报的主要财务数据整段丢失**。标题是「六、 **公司**主要会计数据和财务指标」
  (比年报多个「公司」),括号还是半角 —— 金山与华特 2026 半年报双双 MISS,
  落到下游就是「半年报没披露主要财务数据」,而公司披露了。
- **`capital_flow` 的降级提示从未真正触发**。`pro.block_trade` 是 `functools.partial`,
  没有 `__name__`,v8.7 用 `getattr(fn, "__name__", str(fn))` 取名,key 存成一长串 partial repr,
  于是 `_CALL_ERRORS.get("block_trade")` 永远取不到 —— 写了的提示一次没出现过。
  同时补上第二种降级:**调通了但返回 0 行**(积分不够 / 接口不覆盖这只票,Tushare 都是静默空表)。
  §11 现在分两栏写「没调通」与「返回 0 行」;龙虎榜按日迭代的空表是常态,不计入。

**Added**

- **红旗「投资收益占营业利润过高」**(🟡 中 / 🟠 高,归①质地)。
  原有的「非经常性损益占比过高」是拿 `利润总额 − 营业利润` 估的,只抓得到**营业外**收支;
  而投资收益、公允价值变动**计在营业利润里面**,整块从缝里漏过去:
  金山 2026H1 投资收益 19.14 亿 / 营业利润 27.45 亿 = 70%(上年同期 17%),净利润同比 +237%,
  而营业外收支只有 0.21 亿 —— 老规则一声不吭,ROE、净利率、**同行分位**全被顶高。
  证据句带同比,把「今年突然」和「一直这样」分开。
  手册侧同步:`node-quality.md` §②「赚钱质量真吗」加一条,写手看到净利润跳增先对这个比。
- **合同负债一族进采集白名单与 §2 快照**:`contract_liab` / `contract_assets` /
  `deferred_inc` / `defer_inc_non_cur_liab`。新准则(2020)后预收款重分类到 `contract_liab`,
  老字段 `adv_receipts` 基本全空(金山 23 期里 22 期为空,而 contract_liab 2025 年末 25.99 亿)——
  只看老字段,订阅制 / 预收制公司会被读成「没有递延收入」,而这恰恰是它们最有用的先行指标。
  资产负债表缓存键加了字段列表指纹,免得改完白名单还命中少几列的旧缓存。
- **`oth_cur_liab` / `oth_ncl` 进白名单**(Phase 2 精读打出来的第五个洞)。
  「其他非流动负债」不是杂项兜底:A 股常把**非流动的合同负债**整笔塞在这里。
  金山 2026H1 资产负债表「合同负债」27.64 亿,而 oth_ncl 的 12.38 亿明细全额也是合同负债
  (附注七·52)—— 真实递延收入池 40.02 亿,只读合同负债一行**低估 45%**,
  订阅制公司的多年期大单恰恰都藏在这里。标签直接写「可能含合同负债, 回附注核」。


**Added — 判断链跑完全程后又补的六条(同属 v8.10,金山报告已上站 `d26aca1`)**

- **lint R18 红旗清单与审计同步**(fail)。`audit_report.json` 重跑后忘了重出 `red_flags.json`,
  写手面板就引不到新红旗的 id。装配层是**直接从 audit 重算**红旗的,所以**成品不漏** ——
  漏的只是写手作判断时看到的那份,面板哑一格,不触发任何既有规则。**没有这条规则没人会发现。**
  写手提名不算覆盖;没有 `red_flags.json` 判 skip 而非 fail;报错自带补跑命令。
- **R3 误报收口**:个位数带一位小数的百分比不进 home 追踪(③的无风险利率 1.8% 与①的
  「非经常损益只占归母 1.8%~5.4%」撞成一条,照要求加出处就是写假话)。
  折现率分档、股息率、资本化率天然挤在 0~9.9%。放宽前在三份现役报告上试算:少判 0 条真命中。
- **节点手册 YAML 示例改块式**:③赔率与④路径各因流式映射 `{condition: …, triggered: false}`
  被中文值里的半角逗号切断而丢一轮 —— 而「可核销阈值」几乎必然带逗号(「<40 亿, 或毛利率…」)。
  根子在手册示例本身就是流式写法。
- **附录C 技术面不再替⑤决策发号施令**:`technical_analysis.py` §4 是 v7 遗留的「应用指南」,
  写着「是加仓时机」「应减仓或止损」,与同一份报告⑤的 0 仓位直接冲突。改为只描述状态与价位 +
  明写「档位只归⑤决策」。**不做成 lint 规则**:R8 只扫节点正文,扩到附录会误伤资金面/舆情底稿里
  对券商观点的**引述**;缺陷源头是自家模板在下指令,判据钉在脚本产出上。
- **reviewer 手册:开「补一条清单/记名」类 FIX 必须连形状一起给**(几行几列)。
  R1 让④补齐图脚指向的左尾清单,只说要写、没说形状,章预算就把它挤成一段 17 个数的散文,
  R2 又被 R11 命中。**这是 R11 的根因回路。**
- **人话词典补 `verdict → 判定`**(与 `anchor_range` 同级),**面板 `value` 只放数字 / `note` 与
  「指标选择理由」都是读者向**:⑤三格表依据列四行全写「②状态 verdict」是字段名直译;
  面板 note 里漏出「命中脚本红旗…触发值 0.695」;指标选择理由 230 字把五格瓦片顶到第二屏。

**Changed — 两条写进手册的评审教训**

- **同一名词有两种口径时,第一次用它的那一章要给对照表**(链手册 §4.5)。
  机检对这件事是瞎的:R3 按数字串认 home,而口径之争的特征恰恰是**同名不同数**,机器不会响。
- 引用别节点「用了哪一档」时,**档位对了不等于期别对了**;把证据从 A 降级并让它引用 B 的定级时,
  要同时查 B 自己有没有在别处用高一级的措辞(降级会把定级权集中到 B)。

测试 491 → 524。
---

## [v8.9] — 2026-09-16 — 清空遗留清单:附录缺口 / R17 / 语义归属 / 红旗 id

> 这四条都是「记了很久、单独看都不急」的欠账, 一次做完。清单到此清零。

**Added**

- **R17 已兑现倍数不超增长退出(fail)**:按**自身历史倍数**给已兑现利润定价时,
  该倍数不得高于基准情景的退出倍数 —— 否则等于给不增长的利润付了比增长还贵的价,
  F 里含了增长、N 被低估(华特 R1 实测:低端 40x vs 基准退出 35x, 改 32.8x 后低端 42.2 → 35.3 元)。
  R5 只查两端不倒置, 这种「同向但内部不自洽」它看不见。
  ⚠️ 判据**只卡按自身历史倍数定价的分部**(依据里写着「行情前 / 自身历史 / 分位」)。
  按同业倍数给分部定价是另一回事:东山电子电路 30x、光模块 40x 都高于它的基准退出 20x,
  旭创 25x 高于 18x —— 那是「当期同业倍数」对「五年后的终值倍数」, 本就不可比,
  写粗了会把两份已发布的报告判红。三份现役报告实测零命中。

**Fixed**

- **`technical_analysis.md` 没挂任何附录** → 并进附录C(标题改为「舆情、资金与技术面底稿」)。
  后果不是少一张表:阶段高点、异动启动价、支撑位这些价位**无处可引**, ④ 只能自己安家,
  R3 也无从要求出处(华特④路径报出)。
- **红旗 id 含当期数值**:audit 的 signal 自带值(「Z=8.767」「F=4/9」「M=-2.726」),
  旧写法把整句哈希进 id —— 数据一刷新 id 就变, 同一条红旗每期都被算成「解除一条 + 新增一条」,
  增量复查的红旗 diff 全是噪音。现在 id 只取 signal 的**文字骨架**(数值剥掉)。
  票 09 当时判「会破坏在途引用」而单独切票 —— 实测三家节点里存着的 `red_flag_ref`
  指的都是**不含数值**的 signal(`dupont-96a180` / `buffett-quality-6a72c0`), 归一化不动它们,
  迁移零成本;三家 audit 全量试算零 id 撞车。

**Changed — R3 的语义归属:改手册, 不改规则**

- R3 只能认「第一个不带出处写出它的章」, 而写作顺序是依赖图波次(①③ → ②④ → ⑤),
  与语义归属天然错位:①质地在第一波就会写到增长拆解的数字, home 于是判给①,
  ②状态反过来只能引用它(华特实测:氦气占收入增量 50.8%、同比 +133% 都落在①名下)。
- **没有把它做成机检** —— 机器看见的是数字串, 认不出「这是增长拆解还是盈利能力」,
  硬猜只会变成又一条高误报规则(票 12 刚教过)。改为:链手册 §4.5 给出**语义归属表**
  (五族数字各自的家), ①的手册写明「增长拆解的数字**写的时候就标(②状态)**」。
  写手一标出处, home 自然落到语义主人身上, 规则一行不用动。

- **`compare status` 只比基准日**:同一个 run **原地重装配**(改措辞、改红旗 id、换附录)不动 `report_date`,
  于是它一路答「已是最新」, 而线上对比页还印着 09-02 的旧卡片 —— 实测 pcb-optics 正是这样停在旧叫法上。
  成员快照现在带**内容指纹**(覆盖并排页真正消费的那几样, 不含随天数漂移的字段),
  status 同时比指纹, 基准日没变也能报「原地重出片」。对比页已按新内容重装配、重出片、上站。

**Notes**

- 491 项测试绿(新增 13 条:R17 ×5、附录C 挂载 ×1、红旗 id ×5、对比页内容指纹 ×2)。
- 三份现役报告按新附录与新 id 重出片并重新发布;对比页 `pcb-optics` 同步刷新。

---

## [v8.8] — 2026-09-15 — R11 散文密度多样本标定(票 12,实现票全清)

> R11 的阈值原来是拿**东山一份**报告调出来的, 必然过拟合 —— 票 12 存在的理由就是这句。
> 这次用**三份 v8 报告 + 景嘉微 v7(人工认可的负样本)共 533 段**逐段人工标注
> 「该是表 / 该是散文」(判据 = 并列项之间有没有共同的列), 每条判据单独算精确率。

**Changed — 按标定数据删判据, 不调参**

| 判据 | 命中 | 真阳 | 误报 | 精确率 | 处置 |
|---|---:|---:|---:|---:|---|
| 期间对比 A → B | 6 | 6 | 0 | 100% | 原样保留 |
| 编号项 | 9 | 3 | 6 | 33% | 掩掉假编号后 60%, 保留 |
| 并列项(顿号 ≥3 且数字 ≥4) | 24 | 6 | 18 | 25% | **删除** |
| 超长(>200 字) | 41 | 7 | 34 | 17% | **删除** |
| 核销清单(✓/✗/兑现/证伪) | 5 | 3 | 2 | 60% | 新增 |
| 逐项短并列(每项 ≤18 字带数 ≥4 项) | 0 | 0 | 0 | 零噪音 | 新增 |

**整体 56 命中 / 23% → 12 命中 / 75%**,真阳 13 → 9。对一条人要读的 warn 级定位器,
噪声比漏报更致命:五条里四条是误报,人就会开始整条无视它。

- **误报的头号形状是判定句** —— 它天生带并列与数字, 却是全章最该是散文的一句。
  「中文散文本来就用顿号」这件事, 宽口径并列项确实学成了特征(票里的预判被数据坐实);
  试过「带数字的段 ≥4」想救它, 精确率反而掉到 14% → 按票 12 的规矩:压不下去就删。
- **编号项的误报全部来自假编号**:红旗引用「🟠③④」、节点引用「③赔率」、小节号「(5.4)」。
  这是正则漏洞不是判据站不住 —— 掩掉后精确率翻倍且三条真阳一条没丢。
- **`×` 不能当核销标记**:它同时是乘号(旭创图例「208.0 亿 ×25x」因此被误判)。
- **表下注豁免扩边界**:原来只豁免紧跟表格的一行, 而真实的注常隔着空行或写成
  「> 注 3(DCF 口径):」—— 旭创注 1、华特注 3 都因此被误报, 现按「注 N」开头整段豁免。

**Unchanged — 正向断言维持 warn, 不升阻断**

- 「每章至少一段 ≥80 字、数字 ≤2 的纯判断散文」:15 章里 14 章达标, 唯一不达标的是**东山③赔率** ——
  这正是它的误伤场景(估值章每句话都带锚与倍数)。升为 fail 会阻断一个合法的估值章, 所以不升。

**Notes**

- 478 项测试绿(R11 新增 6 条标定回归:判定句不再误报 / 引用标记不算编号 / 注 N 豁免 /
  核销清单 / 逐项短并列 / 长推理不因长被报)。
- 实测三份报告 R11:东山 6 处、旭创 **18 → 3 处**、华特 1 处;华特那处(③情景表合计行塞了三个每股值)
  与 R3 交付评审**独立**点出的是同一处 —— 判据和人看到了同一个问题。
- 标定数据、标注与脚本入库:`.scratch/v8-implementation/r11-calibration/`。
- **票 12 关闭 = v8 实现票 01-12 全部完成。**
- ⚠️ 语料缺口:三家都不算轻资产(票里希望的「一家轻资产」没凑齐), 负样本是 v7 九章结构。
  结论对「判定句 / 权重理由 / 表下注」三类误报是稳的, 对轻资产公司未验证。
