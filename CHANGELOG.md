...
- **估值尺图例**「SOTP / DCF 两端」→「低端只认已赚到的钱 · 高端按三种前景概率加权」。
  中间一版写成「高端允许按常规增速放量」—— 那是基准情景一条路,不是三情景加权,放在每份报告上都说错
  (华特 R2 交付评审抓到)。两端算法由契约钉死(手册:低端 = 只认已兑现的分部加总,高端 = 三情景概率加权;
  R12 把两端分别绑到 `derivation.sotp` / `dcf` 的每股值),所以一句固定说明对每份报告都成立,不必新增字段。
...
- 447 项测试绿(skipped=7;新增 20 条回归:a 不套 a、旧契约上版仍出变化区块、缺消费字段仍被拒、
