...
一个跑在 Claude Code 里的 slash command（`/company-analysis`）。给它一个公司名（可选股票代码），它会自动采集结构化财报数据、强制精读年报/季报 PDF 原文、跑 11 个大师级会计审计框架，然后用 4 个串行写手分工写出一份 **8 章节** 的投资分析报告，再经 3 个评审 agent 并行打分 + 修正循环，最后渲染 HTML 并发布到 GitHub Pages。

核心定位：**用确定性数据 + 数学推导对抗"伪量化"和"AI 编故事"**。所有关键数字带来源标签（`[Tushare:income.revenue]` / `[PDF:q3_2025, P.4]`），所有估值走 DCF 概率加权，所有红旗由脚本机械扫出。
...
| **数学推导优先** | 估值与回报以可独立验算的 **DCF 概率加权** 为唯一锚，可比 PE/PB 仅作交叉验证；逻辑猜测不得替代算术。 |
...
| **8 章节单一事实源去重** | 每个事实只在一处展开（评分即证据索引、估值与回报共用同一套情景概率），章节间不重复造数。章节边界真理源：`scripts/assemble_report.py:PART_EXPECTED_SECTIONS`。 |
...
Phase 2 文档精析        （主 agent 自跑：精读 PDF 关键段落，提取原文引用）
   ↓
Phase 3 综合分析与报告  （phase3-part{1,2,3,4} 4 个串行写手 → assemble_report 拼成 8 章节）
   ↓
Phase 6 审核发布        （anti_lazy_lint 4 条机械规则 + reviewer-{narrative,valuation,redflag} 3 并行 + 修正循环 + build_html + GitHub Pages）
...
**8 个 sub-agent**：`data-collector`（1）+ `phase3-part{1,2,3,4}`（4 个串行写手，§一执行摘要由 part1 在链尾汇总）+ `reviewer-{narrative,valuation,redflag}`（3 个并行评审）。
...
| 章节 | 内容 |
|------|------|
| **§一 执行摘要** | 一句话结论 + 估值锚 + 综合评分 + Top3 风险/机会 + 投资方向判定（决策仪表盘，读者只读本节即可掌握结论）。字段定义见 `assets/templates/exec-summary-schema.md`。 |
...
| **§五 估值与回报** | DCF 概率加权情景 + 估值锚 + 可比 PE/PB 交叉验证 + 技术面位置 + 投资回报测算（与情景共用概率）。 |
| **§六 风险与红旗审计** | 致命看空快筛 6 项 + 11 框架审计红旗汇总 + 致命看空论证（集中回答"什么会杀死这笔投资"）。 |
| **§七 舆情与市场情绪** | 看多/看空声音各 ≥ 3 条 + 资金流向信号（北向 / 融资融券 / 主力）。 |
| **§八 数据来源与信息缺口** | 三类来源分组（Tushare / PDF / WebSearch）+ 信息缺口与尽调优先级 ≥ 3 条。 |
...
├── phase3-part{1,2,3,4}.md        # 4 写手分稿
├── {公司}-analysis-{date}.md      # ⭐ 主报告（8 章节，assemble 拼接）
...
├── LICENSE                     # MIT
├── .env.sample                 # 环境变量模板
├── SKILL.md                    # ⭐ 协调器主智能体（调度 8 sub-agent）
├── install.sh / uninstall.sh   # 一键安装 / 卸载
│
├── agents/                     # 8 个 sub-agent 定义
│   ├── data-collector.md           # Phase 1 数据采集
│   ├── phase3-part1.md             # 写手：§一 执行摘要（链尾汇总）
│   ├── phase3-part2.md             # 写手：§二 基本面 / §七 舆情
│   ├── phase3-part3.md             # 写手：§三 行业 / §四 评分 / §五 估值
│   ├── phase3-part4.md             # 写手：§六 风险红旗 / §八 来源缺口
...
│   ├── valuation-frameworks.md     # Damodaran 估值 + SOTP
│   ├── search-strategy.md          # WebSearch 辅助规范
│   └── html-template-guide.md      # HTML 可视化规范
│
├── assets/
│   ├── templates/
│   │   ├── report-skeleton.md      # ⭐ 8 章节严格骨架
...
└── scripts/                    # ⭐ Python 数据层
    ...
    ├── assemble_report.py      # ⭐ Phase 3 拼接成 8 章节
