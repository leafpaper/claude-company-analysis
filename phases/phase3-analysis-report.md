...
- **4.11 状态评估**（★ v7.0 新增，机制原料，供 §七 7.1 合成；权威定义见 `references/investment-decision-core.md`）——5 个机制块，**每块出判定/数字**：
  - **【λ 与证据临界密度】**（泊松+贝叶斯）：λ 定义（按资产类型）+ 核心状态跳变事件 + λ↑信号（**★分部级，注明是否被集团稀释**）+ 独立证据 vs 叙事回声逐条表 + 是否达临界 + 临界点 + 贝叶斯三问（后验提高还是注意力提高 / 独立证据还是回声 / 价格是否过度反映）。
  - **【身份切换·升级基本面5元组】**（喊线）：P 几 + 旧→新身份候选 + 是否已被市场重命名 + 5 元组逐项（生产函数位置/约束释放/权威认证/证据再验证节奏/定价充分度）。
  - **【四层验证·权威认证】**（喊线）：①能一句话传播 ②真实产业约束接棒 ③权威节点认证（权威分发 vs 券商回声）④价格买完完美未来（详见§五）——逐层 ✓/✗ + 依据。
  - **【右尾识别·幂律来源·左尾预警】**（十倍股+幂律）：右尾清单逐件（需求/提价权/FCF/利润率/估值起点）+ 幂律来源 + 是不是"幂律生成器节点"。
  - **【无记忆性检查】**（泊松）：本标的买入逻辑是否落入"等久了该涨"幻觉——**禁止把"跌久了/沉寂久了/估值压久了/讲多年该兑现"当买入理由**（anti_lazy_lint Rule 7 机械拦截），买入理由须为 λ↑ 或证据斜率变正。
  - → **状态后验初判**（供 §七 7.1）：↑变好+证据确认 / ↑变好但仅注意力·未确认 / 横盘 / ↓变差。
...
- **5.3 ΔP 传播因子分解**（喊线）：把本轮上涨拆成 Authority / Narrative Compression / Attention Velocity / Marginal Buyer / Peer Resonance / Gamma·Flow / Right-tail − (PFPC/Crowding/Valuation Exhaustion)，逐因子判强弱 → 是"权威认证驱动"还是"散户拥挤+估值耗尽驱动"。
...
承接骨架 §七 的 placeholder。**这是各篇理念的合成结论 + 全报告 punch line**——用"价值 = 状态后验 × 赔率 × 路径可承受性"（乘法，任一为差即拉低）合成 §四 4.11（状态）、§五（赔率）、§六 6.4（路径）。权威定义见 `references/investment-decision-core.md`。

- **7.1 状态后验合成**（引 §四 4.11）→ {↑变好+证据确认 / ↑变好但仅注意力·未确认 / 横盘 / ↓变差}。
- **7.2 赔率合成**（引 §五 5.7）→ {便宜 / 合理 / 已price-in / 买完完美未来}。
- **7.3 路径可承受性合成**（引 §六 6.4 + §五 5.6）→ {可承受 / 高尾险·不可承受}。
- **7.4 决策合成**（★ anti_lazy_lint Rule 6 机械检查本节含「决策三元组」「行动档位」「证伪」+ 六档之一）：
  - **决策三元组** [状态后验 | 赔率 | 路径]；
...
- **7.5 与 §一 对齐**：7.4 的决策三元组/三分/行动档位将被 part1 照抄进 §一 决策结论，务必清晰可抄。
...
2. **估值锚**（§五 综合估值锚 / 每股 / 相对当前股价折让%；v7.0 = P=F+N + 反向DCF + 叙事SOTP，DCF 仅交叉验证）
3. **综合评分**（`X.XX/10` + 数据置信度；= §四 加权评分表加总，允差 ≤ 0.05；★ **标注「基本面静态快照」，非投资结论**，方向看第 7 字段）
4. **三大风险 Top 3**（每条 ≤ 50 字，每条对应 ≥ 1 个 audit 🔴/🟠 红旗或 §六 快筛触发条款）
5. **三大机会 Top 3**
6. **核心非共识判断**（1-3 条，可选；由分析师直接给出本报告与市场共识的关键分歧，纯标题/短句）
7. **决策结论**（★ v7.0，来自 §七 7.4，**本报告权威结论**，须照抄一致）：决策三元组 [状态 ｜ 赔率 ｜ 路径] + 三分结论（好公司?/好下注?/好价格?）+ 行动档位（六档之一）+ 该等什么。**不得用旧"投资方向综合判定/看多看空"代替。**

★ **§一 全程干净叙述,禁止内联来源标签**（`[data_snapshot…]`/`[peer_analysis…]`/`[metrics.json…]`/`[§X]`/`[缺口#N]`/`[WebSearch/Tushare/PDF:…]`）——执行摘要只给结论与逻辑,出处放 §二–§九（`anti_lazy_lint` Rule 5 机械拦截）。
