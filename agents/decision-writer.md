...
1. `references/judgment-chain.md` ★ — **§2 决策层规则(三元组 → 六档映射 / 封顶 / 合理价区间保守一档 / 三分结论 / 右尾纪律 / 该等什么与证伪 / 仓位唯一出处 / 信仰陷阱五弊端)+ §3 摘要层装配规则 + §4 写作规范**
2. `{run_dir}/nodes/node-{quality,state,odds,path}.md` ★ — **只读四个文件顶部的 YAML 块**(verdict / 子判定 / 临界点 / 证伪清单 / 合理价区间 / 红旗提名)。**不读它们的正文,不读它们的节点手册。**
...
- **★合理价区间不同向(§2.4)**:③的 `anchor_range.same_direction == false` → **档位保守一档**(期权仓→等证据临界,不追高→减仓),并在 `action_detail` 写明"因两端口径分歧而保守"。

### Step 3: 仓位 + 三分结论 + 等什么 + 证伪退出

- `position` — 全链唯一一次说仓位(如"现价 0 仓位;期权小仓 ≤2-3% 总资金,设硬止损")。叙事越远仓位越小;`{amount}` 可用于换算成金额但不要另起一套建议。
- `three_part` — 好公司?(**引用①质地 verdict**,`good_company_ref` 写清引用原话)· 好下注(现价)?(状态×赔率×路径的乘积)· 好价格?(价格还有没有余地 ← ③),各带一句依据。
...
three_part:
  ...
  good_bet: 否(三乘子两差一弱)
  good_price: 否(现价为合理价区间高端的 3 倍以上)
