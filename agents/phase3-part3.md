...
### ★★ v7.1 写法铁律（最高优先级，先读 `phases/phase3-analysis-report.md` §写作风格契约 + `references/investment-decision-core.md` 人话词典）
- **全说人话**：§四 4.11 / §五 正文**不写裸术语**(λ/P=F+N/embedded obligation/证据临界…)，用大白话；框架名只在小标题小括号淡化标注。没读过那五篇文章的人要能看懂。
- **结论先行**：每个分析块**第一行**给加粗"一句话"结论，再给证据。
- **正文无来源标签**：§四 4.11 / §五 不放内联 `[data_snapshot…]`/`[§X]`/`[Tushare:…]` 等，来源进 §九。(§五 机械拦截 Rule 5；§四 4.11 由 reviewer 把关；★ §四 10 维度"维度证据"仍保留 audit 标签做红旗闭环——别去掉那里的标签。)
- **一个论点一个数字 + 排版三段式**：核心数(分部占比/估值分位/散户倍数)只说一次；每块=粗体结论→2-4 短支撑→紧凑表。
- **§四 必做四维体检表**(懂财报/懂叙事/懂估值/懂热点) + 证据"实锤/传闻"表。
