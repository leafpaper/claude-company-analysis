...
def _format_markdown(df: pd.DataFrame, s: dict) -> str:
    ...
    lines.extend([
        ...
        "## §4 这几个价位怎么用(**档位只归⑤决策**)",
        "",
        "本节只说技术面**现在是什么状态**、价位在哪里。要不要买、买多少、什么时候卖,"
        "全链只有⑤决策能说 —— 这里给的是它和④路径引用的坐标,不是建议。",
        "",
        ...
        "",
        "**支撑阻力位是坐标,不是扳机**:",
        f"- 当前价 {s['close']} 元,近 60 日支撑 {s['support_60d']} 元(相距 {s['dist_to_support_60']}%)",
        f"- 近 60 日阻力 {s['resist_60d']} 元(上方 {s['dist_to_resist_60']}%)",
