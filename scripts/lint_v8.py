...
#
# **个位数带一位小数的百分比同理**(金山 688111 实测):③的无风险利率 1.8% 与①的
# 「非经常损益只占归母 1.8%~5.4%」撞成一条 —— 折现率分档、股息率、资本化率这些数天然挤在
# 0~9.9% 这一小段里, 撞车是常态而语义毫不相干, 加出处同样只能写假话。
# 放宽前在三份现役报告(东山 / 旭创 / 华特)上试算:少判 0 条真命中, 只去掉这一条误报。
_COLLIDING_PCT = re.compile(r"\d{1,2}%|\d\.\d%")
