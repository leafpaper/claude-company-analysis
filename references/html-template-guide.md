...
| 组件 | 用在 | 意图 |
|------|------|------|
| **评分环(Score Ring)** | §一 / §四 | SVG 圆环把"综合评分 / 10"可视化 —— 远比数字本身更"一眼入眼" |
| **维度条(Dimension Bar)** | §四 | 10 维度进度条 + Tier 颜色(蓝 = 权重 15% / 紫 = 10% / 琥珀 = 5%)→ 直观看哪些维度权重大 |
| **情景卡(Scenario Card)** | §五 | 4 情景(乐观/基准/悲观/**最差**)。最差卡用深黑色背景 —— 视觉强调"尾部风险" |
| **期望回报条(Expected Return)** | §五 | 深蓝渐变大字 —— 最终加权收益率是报告"一句话结论"的数字背书 |
| **团队名片网格(Team Grid)** | §二 / §四 维度 5 | 头像圆圈(姓氏首字)+ 姓名 + 职位 → 比纯文本易记 |
| **风险项(Risk Item)** | §一 / §六 | 左边色条 + 圆点 + badge →红黄绿分级一眼可辨 |
| **时间轴(Timeline)** | §二 | 水平节点时间轴 —— 经营动态 / 事件演进 / current 节点高亮 |
| **情绪量表(Sentiment Bar)** | §七 | 红到绿渐变条 + 指针 —— 市场情绪定位更直观 |
| **估值区间(Valuation Range)** | §五 | DCF/可比/PB 三种方法的区间叠加显示 —— 看重叠区域就是共识 |
